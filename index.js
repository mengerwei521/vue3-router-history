...
class routerHistory {
  constructor(options) {
    ...
    this.isGo_num = null; //是不是back / go(-x)回退
   ...
   */
    ...
    this.router.afterEach((to, from) => {
      if (to.name !== from.name) {
        ...
        this.isGo_num = null;  // 重置go/back 跳转  使用手机物理回退键该变量会莫名添加【没有找到原因】
      }
...
export default {
  install: (app, options) => {
    console.log(app, options, 'optionsoptionsoptions')
    app.config.globalProperties.$routerHistory = new routerHistory(options)
  }
}